 ...
 *
 * <p>HTML preview endpoints return Thymeleaf-rendered HTML (useful for development / tweaking
 * templates). PNG endpoints convert the same HTML to a raster image using OpenHTMLtoPDF and return
 * the bytes with {@code Content-Type: image/png}.
 *
 * <h2>Endpoints</h2>
 *
 * <pre>
...
public class BannerController {
   ...
   *
   * <p>Looks for the photo in {@code /static/images/speaker/{id}.{jpg|png|jpeg}} on the classpath.
   * If no photo is found the client is redirected to the default placeholder image.
   *
   * @param id speaker UUID
   ...
   */
  @PostMapping(
      value = "/api/banners/generate-speakers",
  ...
  private Speaker requireSpeaker(UUID id) {
    return speakerRepository
        .findById(id)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Speaker not found: " + id));
  }
  ...
  private Talk requireTalk(Long id) {
    return talkRepository
        .findById(id)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Talk not found: " + id));
  }
