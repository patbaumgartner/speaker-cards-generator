...
/**
 * Converts an HTML string to a PNG image using OpenHTMLtoPDF for HTML→PDF
 * rendering and Apache PDFBox for PDF→PNG conversion.
 *
 * <p>Static resources (fonts, background images) referenced in the HTML with
 * absolute paths such as {@code /static/images/background.png} are resolved
 * via the running application server at
 * {@code http://localhost:{server.port}}.  The caller must therefore ensure
 * the application is fully started before invoking this converter.
 */
@Component
public class HtmlToPngConverter {
     ...
     *
     * <p>The HTML is rendered at screen resolution (96 DPI) and the first page
     * of the resulting PDF is then converted to PNG.
     *
    ...
    /**
     * Renders the given HTML to PDF bytes.  Absolute paths beginning with
     * {@code /} in the HTML are resolved against
     * {@code http://localhost:{serverPort}}.
     *
    ...
    /**
     * Converts the first page of a PDF to a PNG byte array at 72 DPI
     * (1:1 pixel mapping for banners sized in pixels).
     *
