 ...
 *
 * <p>The import is idempotent: existing speakers and talks are looked up by their primary keys and
 * only missing records are inserted. Speaker profile pictures are downloaded to {@code
  ...
  private Talk createOrUpdateTalk(String[] fields) {
    ...
    try {
      ...
      if (isNew) {
        ...
        log.debug(
            "Found existing talk: {} ({}) with {} speaker(s)",
            talk.title,
            sessionId,
            talk.speakers.size());
      }
