 ...
 * </ul>
 *
 * <p>The import is idempotent: existing speakers (matched by UUID) are updated in place; profile
 * pictures are downloaded once and cached.
 */
@Service
public class DevoxxImportService {
   ...
   * application.properties} ({@code app.devoxx.api.event-id}) and saves them to the database.
   *
    ...
    try {
      ...
      for (DevoxxSpeakerDto dto : speakers) {
        try {
          ...
          log.error(
              "Error saving speaker {} {}: {}", dto.firstName, dto.lastName, e.getMessage(), e);
        }
      }
  ...
  /**
   * Extracts just the LinkedIn handle from a full URL if necessary. Returns {@code null} if the
   * input is blank.
   */
  ...
  private void downloadProfilePicture(UUID speakerId, String pictureUrl) {
    try {
      ...
      log.error(
          "Error downloading profile picture for speaker {} from {}", speakerId, pictureUrl, e);
    }
  }
  ...
  /**
   * JSON DTO for a speaker returned by the Devoxx mobile API. Unknown fields are silently ignored
