 ...
 *
 * <p>The API endpoint format is:
 * <pre>
 *   GET {baseUrl}/events/{eventId}/speakers
 * </pre>
 *
 * <p>Supported event IDs:
 * <ul>
 *   <li>{@code vdz26} – Voxxed Days Zürich 2026</li>
 *   <li>{@code vdt26} – Voxxed Days Ticino 2026</li>
 *   <li>{@code vdcern26} – Voxxed Days CERN 2026</li>
 * </ul>
 *
 * <p>The import is idempotent: existing speakers (matched by UUID) are
 * updated in place; profile pictures are downloaded once and cached.
 */
@Service
public class DevoxxImportService {
    ...
    public DevoxxImportService(DevoxxApiConfig devoxxApiConfig,
    ...
    /**
     * Fetches speakers from the Devoxx API using the event ID configured in
     * {@code application.properties} ({@code app.devoxx.api.event-id}) and
    ...
    public int importSpeakers(String eventId) {
        ...
        try {
            DevoxxSpeakerDto[] speakers = restClient.get()
                    .uri(url)
                    .retrieve()
    ...
    private void saveSpeaker(DevoxxSpeakerDto dto) {
        ...
        log.debug("{} speaker: {} {} ({})", isNew ? "Saved" : "Updated",
    ...
    /**
     * Extracts just the LinkedIn handle from a full URL if necessary.
     * Returns {@code null} if the input is blank.
     */
    ...
    /**
     * JSON DTO for a speaker returned by the Devoxx mobile API.
     * Unknown fields are silently ignored to remain forward-compatible.
     */
